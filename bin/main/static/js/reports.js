...
function generateReport() {
    ...
    getHTTPService(`/reports/${currentReport}?start=${start}&end=${end}`, 'GET', 'json')
        .then(data => {
            showLoading(false);
            if (data && data.error) {
...
function renderReport(type, data, start, end) {
    ...
    <div style="padding:16px 20px; border-bottom:1px solid var(--border-color,#e2e8f0); display:flex; justify-content:space-between; align-items:center;">
        <div>
            ...
            <span style="font-size:0.8rem; color:var(--text-muted,#64748b); margin-left:10px;">${start} to ${end}</span>
        </div>
